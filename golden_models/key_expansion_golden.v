module key_expansion_golden #(parameter nk=4,parameter nr=10)(key,w);
...
/*
	...
	-The example:
		...
		3) After performing (temp = SubWord(RotWord(temp)) xor Rcon[i/Nk]) or (temp = SubWord(temp)), We would perform
			(new_reg = w[(128*(nr+1)-(nk*32))+:32] ^ temp) which is (new_reg=w[1280+:32] ^ temp) in the first round,
			where w[(128*(nr+1)-(nk*32))+:32] here is equivelent to w[i-Nk] in the pseudo-code. Now we have the new_reg
			generated key word (new_reg) and we need to add it at the end of (W) array.
		4) We would shift W by 32-bit to the left to empty space to the new_reg generated key word.
		5) w = {w[0 : (128 * (nr + 1) - 32) - 1], new_reg} is w={w[0:1375],new_reg} where w now contains w={1247*{0}:key:new_reg} 
			where 1247*{0} means that the first 1247 bits are all zero valued and they are followed by the original 
			128-bit key, which are followed by the new_reg generated 32-bit key.
...
//The first [(nk*32)-1 ]-bit key is stored in W.
	...
	// We would shift W by 32 bit to the left to add the new_reg generated key word (new_reg) at its end.
	w = w << 32;
