module ALU_control(
    ...
    always @(*) begin
        casez({ALUOp, funct7[5], funct3})
            ...
            // BGE: signed less-than; Branch_prediction inverts it to get A>=B.
            // (was 4'b0100 = signed A>B, whose inverse is A<=B -- wrong.)
            {3'b011, 1'b?, 3'b101}: ALU_control = 4'b0010;
            {3'b011, 1'b?, 3'b110}, {3'b001, 1'b?, 3'b011}, {3'b000, 1'b0, 3'b011}: ALU_control = 4'b0011;
            // BGEU: unsigned less-than; inverted by Branch_prediction to A>=B.
            // (was 4'b0101 = unsigned A>B, whose inverse is A<=B -- wrong.)
            {3'b011, 1'b?, 3'b111}: ALU_control = 4'b0011;
